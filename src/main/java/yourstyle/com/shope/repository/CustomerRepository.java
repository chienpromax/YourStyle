...
public interface CustomerRepository extends JpaRepository<Customer, Integer> {
	...
	Page<Customer> findByFullnameContaining(@Param("customerId") Integer customerId, @Param("fullname") String fullname,
			Pageable pageable);

	@Query("SELECT c FROM Customer c WHERE NOT c.customerId = :customerId AND c.phoneNumber LIKE CONCAT('%',:phoneNumber,'%')")
	Page<Customer> findByPhoneName(@Param("customerId") Integer customerId, @Param("phoneNumber") String phoneNumber,
			Pageable pageable);
