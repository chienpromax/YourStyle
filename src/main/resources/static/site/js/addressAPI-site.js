...
function attachEventHandlers() {
  ...
  selectDistricts.forEach((selectDistrict) => {
    selectDistrict.addEventListener("change", function () {
      ...
      // Kiểm tra nếu parentRow và phần tử floatingcity tồn tại
      if (parentRow) {
        const selectWard = parentRow.querySelector(".floatingward");
        const cityElement = parentRow.querySelector(".floatingcity");
        const cityCode = cityElement ? cityElement.value : null; // Kiểm tra xem cityElement có tồn tại không
        ...
        if (districtCode && cityCode) {
          const city = locationData.find((city) => city.code == cityCode);
          const district = city
            ? city.districts.find((district) => district.code == districtCode)
            : null;
          if (district) {
